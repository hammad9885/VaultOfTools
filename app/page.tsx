...
export default function Home() {
  const categories = [
    { icon: "ti-writing", name: "Writing", count: "312 tools", color: "bg-violet-50 text-violet-600" },
    { icon: "ti-photo-ai", name: "Image Gen", count: "248 tools", color: "bg-pink-50 text-pink-600" },
    { icon: "ti-code", name: "Coding", count: "195 tools", color: "bg-blue-50 text-blue-600" },
    { icon: "ti-video", name: "Video AI", count: "167 tools", color: "bg-red-50 text-red-600" },
    { icon: "ti-chart-bar", name: "Analytics", count: "143 tools", color: "bg-amber-50 text-amber-600" },
    { icon: "ti-microphone", name: "Voice & Audio", count: "121 tools", color: "bg-teal-50 text-teal-600" },
    { icon: "ti-users", name: "Sales & CRM", count: "98 tools", color: "bg-green-50 text-green-600" },
    { icon: "ti-gavel", name: "Legal AI", count: "74 tools", color: "bg-orange-50 text-orange-600" },
    { icon: "ti-heart-rate-monitor", name: "Healthcare", count: "63 tools", color: "bg-rose-50 text-rose-600" },
    { icon: "ti-school", name: "Education", count: "118 tools", color: "bg-cyan-50 text-cyan-600" },
    { icon: "ti-speakerphone", name: "Marketing", count: "204 tools", color: "bg-indigo-50 text-indigo-600" },
    { icon: "ti-robot", name: "Agents & Auto", count: "89 tools", color: "bg-purple-50 text-purple-600" },
  ];
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <section className="relative pt-20 pb-16 px-6 text-center overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-500 text-base max-w-md mx-auto mb-8 leading-relaxed font-light">
            Find, compare and track the best AI tools across every category. Updated daily with live pricing and alternatives.
          </p>
          ...
          <div className="flex items-center max-w-xl mx-auto mb-5 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden focus-within:ring-2 focus-within:ring-emerald-300 focus-within:border-emerald-400 transition-all">
            <i className="ti ti-search text-slate-400 text-lg px-4" aria-hidden="true" />
            <input
              type="text"
            ...
            {[
              ...
              <div key={s.lbl} className="flex items-center gap-8">
                <div className="text-center">
                  <div className="text-xl font-bold text-slate-800">{s.num}</div>
      ...
      <section className="px-6 py-10">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-slate-800">Browse Categories</h2>
          <a href="#" className="flex items-center gap-1 text-sm text-slate-400 hover:text-emerald-600 transition-colors">
            All 89 categories <i className="ti ti-arrow-right text-sm" aria-hidden="true" />
          </a>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {categories.map((cat) => (
            ...
            >
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${cat.color.split(" ").slice(0,1).join(" ")} shrink-0`}>
                <i className={`ti ${cat.icon} text-lg ${cat.color.split(" ").slice(1).join(" ")}`} aria-hidden="true" />
              </div>
              <span className="text-sm font-semibold text-slate-700 leading-tight">{cat.name}</span>
      ...
      <section className="px-6 pb-10">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
          ...
          <div>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-slate-800">Featured Tools</h2>
              <a href="#" className="flex items-center gap-1 text-sm text-slate-400 hover:text-emerald-600 transition-colors">
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${tool.iconBg}`}>
                      <i className={`ti ${tool.icon} text-2xl`} aria-hidden="true" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-bold text-slate-800">{tool.name}</div>
                      <div className="text-xs text-slate-400 truncate">{tool.tagline}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-1.5 flex-wrap">
                      {tool.tags.map((tag) => (
                        <span key={tag} className="text-[11px] px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 border border-slate-200">
                          {tag}
                        </span>
                      ))}
                    </div>
                    <span className={`text-[11px] font-medium px-2 py-0.5 rounded-md ${tool.pricingClass}`}>
                      {tool.pricing}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-0.5">
                      ...
                      <span className="text-xs text-slate-400 ml-1.5">{tool.rating} ({tool.reviews})</span>
                    </div>
                    <span className="text-xs text-slate-400">{tool.updated}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <i className="ti ti-trending-up text-orange-500" aria-hidden="true" />
                Trending
              </h2>
              <a href="#" className="flex items-center gap-1 text-sm text-slate-400 hover:text-emerald-600 transition-colors">
                More <i className="ti ti-arrow-right text-sm" aria-hidden="true" />
              </a>
            </div>
            <div className="flex flex-col gap-2">
              {trending.map((t) => (
                ...
                >
                  <span className={`text-sm font-bold min-w-4.5 ${t.rank <= 2 ? "text-emerald-500" : "text-slate-300"}`}>
                    {t.rank}
                  </span>
        ...
        <div className="relative bg-white border border-slate-200 rounded-2xl px-8 py-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 overflow-hidden shadow-sm">
          ...
          <div className="relative">
            <h3 className="text-xl font-bold text-slate-800 mb-1.5 flex items-center gap-2">
              <i className="ti ti-rocket text-emerald-500 text-xl" aria-hidden="true" />
              Built something amazing?
            </h3>
            <p className="text-sm text-slate-500 leading-relaxed max-w-sm">
              Submit your AI tool to reach 180,000+ monthly visitors. Get listed in 24 hours. Free forever, or go sponsored for premium placement.
            </p>
          </div>
