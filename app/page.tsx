...
export default function Home() {
  // isDark is consumed so Navbar's ThemeToggle re-renders this page too
  const { isDark } = useTheme(); // eslint-disable-line @typescript-eslint/no-unused-vars
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans transition-colors duration-300">
      ...
      <section className="relative pt-32 pb-16 px-6 text-center overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-slate-50 leading-tight tracking-tight max-w-2xl mx-auto mb-5">
            The <span className="text-emerald-600 dark:text-emerald-400">Smartest Way</span> to{" "}
            <span className="text-slate-400 dark:text-slate-500">Discover AI Tools</span>
          </h1>
          ...
          <div className="flex items-center max-w-xl mx-auto mb-5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden focus-within:ring-2 focus-within:ring-emerald-300 dark:focus-within:ring-emerald-700 focus-within:border-emerald-400 dark:focus-within:border-emerald-600 transition-all">
            <i className="ti ti-search text-slate-400 dark:text-slate-500 text-lg px-4" aria-hidden="true" />
            <input
              type="text"
            ...
            {heroStats.map((s, i) => (
              <div key={s.lbl} className="flex items-center gap-8">
                <div className="text-center">
                  <div className="text-xl font-bold text-slate-800 dark:text-slate-100">{s.num}</div>
                  <div className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{s.lbl}</div>
                </div>
      ...
      <section className="px-6 py-10">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Browse Categories</h2>
          <a href="#" className="flex items-center gap-1 text-sm text-slate-400 dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
            All 89 categories{" "}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${bg} dark:opacity-80 shrink-0`}>
                  <i className={`ti ${cat.icon} text-lg ${text}`} aria-hidden="true" />
                </div>
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 leading-tight">{cat.name}</span>
      ...
      <section className="px-6 pb-10">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
          ...
          <div>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Featured Tools</h2>
              <a href="#" className="flex items-center gap-1 text-sm text-slate-400 dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
                View all{" "}
                <i className="ti ti-arrow-right text-sm" aria-hidden="true" />
              </a>
              ...
              {featuredTools.map((tool) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${tool.iconBg} dark:opacity-80`}>
                      <i className={`ti ${tool.icon} text-2xl`} aria-hidden="true" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-bold text-slate-800 dark:text-slate-100">{tool.name}</div>
                      <div className="text-xs text-slate-400 dark:text-slate-500 truncate">{tool.tagline}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-1.5 flex-wrap">
                      {tool.tags.map((tag) => (
                        <span key={tag} className="text-[11px] px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-400 dark:text-slate-500">{tool.updated}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {trending.map((t) => (
                <div
                  key={t.rank}
                  className="flex items-center gap-3 px-3 py-2.5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl hover:border-slate-200 dark:hover:border-slate-700 hover:shadow-sm cursor-pointer transition-all"
                >
                  <span className={`text-sm font-bold min-w-4.5 ${rankColor(t.rank)}`}>
                    {t.rank}
                  </span>
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${t.bg} dark:opacity-80`}>
                    <i className={`ti ${t.icon} text-base`} aria-hidden="true" />
                  </div>
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-200 flex-1">{t.name}</span>
